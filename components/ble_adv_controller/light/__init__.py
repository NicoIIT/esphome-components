...
def deprecated_split_option(value):
    if value == False:
        logging.error(f"'{CONF_BLE_ADV_SPLIT_DIM_CCT}' / '{CONF_BLE_ADV_SPLIT_DIM_RGB}' - This option is DEPRECATED. A new variant zhijia / v2_bl is available to replace it in case of blinking light.")
    else:
        logging.warning(f"DEPRECATION: '{CONF_BLE_ADV_SPLIT_DIM_CCT}' / '{CONF_BLE_ADV_SPLIT_DIM_RGB}' is DEPRECATED, you can safely remove it.")
    return value

def deprecated_secondary_option(value):
    raise cv.Invalid(f"'{CONF_BLE_ADV_SECONDARY}' option is DEPRECATED. It must be replaced by the options:\n\n    type: 'onoff' # only if you have not specified it already\n    index: 1\n\nSee Change log.")
