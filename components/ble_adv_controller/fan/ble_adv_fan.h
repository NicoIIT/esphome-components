...
class BleAdvFan : public Component, public fan::Fan, public BleAdvEntity {
 public:
  ...
  void set_speed_count(uint8_t speed_count) {
    this->traits_.set_supported_speed_count(speed_count);
    this->traits_.set_speed(speed_count > 0);
  }
  ...
  void set_forced_refresh_on_start(bool forced_refresh_on_start) {
    this->forced_refresh_on_start_ = forced_refresh_on_start;
  }
