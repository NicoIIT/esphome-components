...
- **raw_inject**:
  ...
  - Options:
    ...
    - **raw**: the raw command to send, in hexa string format, supported formats:
        ...
        ```
- **cmd**
  - Description: Injects a generic command. **Reserved to experienced developers that are able to understand c++ software.**
  - Options:
    - **id**: the controller id
    - **cmd**: the 1 byte command code as defined in enum CommandType of class BleAdvHandler.
    - **param**: the 1 byte param
    - **args**: the 1 to 3 bytes args as a list

- **custom_cmd**:
  - Description: Injects a custom command directly with the langage of the application used. 
  - Options:
    - **id**: the controller id
  ...
  The simplest way to know the cmd / param1 / args values to be entered is to copy a command generated by a physical remote / phone application. Just setup your ESP32 to listen to the traffic using [ble_adv_handler](../ble_adv_handler/README.md) while you do the action you want to copy, this will produce such logs:
  ```
