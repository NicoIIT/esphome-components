...
bool AgarceEncoder::decode(uint8_t* buf, BleAdvEncCmd & enc_cmd, ControllerParam_t & cont) const {
  ...
  if ( (enc_cmd.cmd == 0x00) && (data->args[1] == 0x00) ) return false;
  // Exclude wrong prefix
  if ( (enc_cmd.cmd != 0x00) && data->prefix != this->prefix_) return false;
  if ( (enc_cmd.cmd == 0x00) && data->prefix != (this->prefix_ & 0x0F)) return false;
