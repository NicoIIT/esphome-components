...
FanLampEncoder::FanLampEncoder(const std::string &encoding, const std::string &variant,
...
FanLampEncoderV1::FanLampEncoderV1(const std::string &encoding, const std::string &variant, uint8_t pair_arg3,
    ...
    : FanLampEncoder(encoding, variant, {0xAA, 0x98, 0x43, 0xAF, 0x0B, 0x46, 0x46, 0x46}),
      pair_arg3_(pair_arg3),
      pair_arg_only_on_pair_(pair_arg_only_on_pair),
      with_crc2_(supp_prefix == 0x00 || forced_crc16_2 != 0),
      xor1_(xor1),
      forced_crc16_2_(forced_crc16_2) {
  if (supp_prefix != 0x00)
    this->prefix_.insert(this->prefix_.begin(), supp_prefix);
...
bool FanLampEncoderV1::decode(uint8_t *buf, BleAdvEncCmd &enc_cmd, ControllerParam_t &cont) const {
  ...
  if (!this->check_eq_buf(this->prefix_.data(), buf, this->prefix_.size(), "Prefix"))
    return false;
  if (data->cmd == 0x28 && !this->check_eq(this->pair_arg3_, data->args[2], "Arg3"))
    return false;
  if (data->cmd != 0x28 && !this->pair_arg_only_on_pair_ && !this->check_eq(this->pair_arg3_, data->args[2], "Arg3"))
    return false;
  if (data->cmd != 0x28 && this->pair_arg_only_on_pair_ && !this->check_eq(0, data->args[2], "Arg3"))
    return false;

  uint16_t seed = htons(data->seed);
  uint8_t seed8 = static_cast<uint8_t>(seed & 0xFF);
  if (!this->check_eq(this->xor1_ ? seed8 ^ 1 : seed8, data->r2, "r2"))
    return false;
  ...
  if (data->args[2] != 0) {
    if (!this->check_eq(this->pair_arg3_, data->args[2], "Arg3"))
      return false;
  }
...
FanLampEncoderV2::FanLampEncoderV2(const std::string &encoding, const std::string &variant,
                                   const std::vector<uint8_t> &&prefix, uint16_t device_type, bool with_sign)
...
std::string FanLampEncoderV2::to_str(const BleAdvEncCmd &enc_cmd) const {
  char ret[100];
  size_t ind = std::sprintf(ret, "0x%02X - param1 0x%02X - args[%d,%d]", enc_cmd.cmd, enc_cmd.param1, enc_cmd.args[0],
                            enc_cmd.args[1]);
  return ret;
}
...
void FanLampEncoderV2::whiten(uint8_t *buf, uint8_t size, uint8_t seed, uint8_t salt) const {
  static constexpr uint8_t XBOXES[128] = {
      0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15, 0x04, 0xC7, 0x23,
      0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75, 0xD0, 0xEF, 0xAA, 0xFB, 0x43, 0x4D,
      0x33, 0x85, 0x45, 0xF9, 0x02, 0x7F, 0x50, 0x3C, 0x9F, 0xA8, 0x51, 0xA3, 0x40, 0x8F, 0x92, 0x9D, 0x38, 0xF5, 0xBC,
      0xB6, 0xDA, 0x21, 0x10, 0xFF, 0xF3, 0xD2, 0xE0, 0x32, 0x3A, 0x0A, 0x49, 0x06, 0x24, 0x5C, 0xC2, 0xD3, 0xAC, 0x62,
      0x91, 0x95, 0xE4, 0x79, 0xE7, 0xC8, 0x37, 0x6D, 0x8D, 0xD5, 0x4E, 0xA9, 0x6C, 0x56, 0xF4, 0xEA, 0x65, 0x7A, 0xAE,
      0x08, 0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 0x9B, 0x1E, 0x87, 0xE9, 0xCE, 0x55, 0x28, 0xDF, 0x8C, 0xA1,
...
bool FanLampEncoderV2::decode(uint8_t *buf, BleAdvEncCmd &enc_cmd, ControllerParam_t &cont) const {
  ...
  if (!this->check_eq_buf(this->prefix_.data(), buf, this->prefix_.size(), "Prefix"))
    return false;
  if (!this->check_eq(this->device_type_, data->type, "Device Type"))
    return false;
  if (this->with_sign_ && !this->check_eq(this->sign(buf + 1, data->tx_count, data->seed), data->sign, "Sign"))
    return false;
  if (!this->with_sign_ && !this->check_eq(0, data->sign, "Sign"))
    return false;
  if (!this->check_eq(crc16, data->crc16, "crc16"))
    return false;
