...
from ..ble_adv_controller import BleAdvController
...
BleAdvCmdTrigger = bleadvremote_ns.class_(
    "BleAdvCmdTrigger", automation.Trigger.template(BleAdvGenCmd)
)
    ...
    DEVICE_BASE_CONFIG_SCHEMA.extend(
        {
            ...
            cv.Optional(CONF_BLE_ADV_CYCLE, default=[]): cv.ensure_list(
                cv.All(cv.ensure_list(cv.percentage), cv.Length(min=2, max=2))
            ),
            cv.Optional(CONF_BLE_ADV_REM_CONTROL): cv.ensure_list(
                cv.use_id(BleAdvController)
            ),
            cv.Optional(CONF_BLE_ADV_REM_PUBLISH): cv.ensure_list(
                cv.use_id(BleAdvController)
            ),
